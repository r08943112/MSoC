...
#pragma HLS RESOURCE variable=cordic_phase core=RAM_1P_BRAM
  ...
  for (int j = 0; j < NUM_ITERATIONS; j++) {
      ...
      //#pragma HLS UNROLL
      int temp_sigma_factor = sigma * factor;
	  #pragma HLS RESOURCE variable=temp_sigma_factor core=Mul
      // Multiply previous iteration by 2^(-j)
      COS_SIN_TYPE cos_shift = current_cos * temp_sigma_factor;
      COS_SIN_TYPE sin_shift = current_sin * temp_sigma_factor;
